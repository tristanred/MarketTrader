 ...
 * document.
 */
export function LedgerRow({ label, children, className }: LedgerRowProps) {
  return (
    <div className={cn('flex items-baseline gap-2', className)}>
      <span className="shrink-0 font-mono text-[10px] uppercase tracking-[0.14em] text-muted">
        {label}
      </span>
      <span aria-hidden className="min-w-[1rem] flex-1 border-b border-dashed border-hairline-strong" />
      <span className="shrink-0 font-mono text-xs text-text-strong">{children}</span>
    </div>
  );
}
