...
import globeTrotter from '../../../src/achievements/definitions/globe-trotter.js';
...
describe('achievement: globe-trotter', () => {
  it('unlocks once distinctSymbolsTradedEver reaches 15', async () => {
    const h = await makeAchievementHarness(globeTrotter);
    await setDistinct(h, 15);
    await fireTrade(h);
    expect(await h.isUnlocked()).toBe(true);
  });

  it('does not unlock at distinctSymbolsTradedEver = 14', async () => {
    const h = await makeAchievementHarness(globeTrotter);
    await setDistinct(h, 14);
    await fireTrade(h);
    expect(await h.isUnlocked()).toBe(false);
    expect(await h.progress()).toBe(14);
  });
});
